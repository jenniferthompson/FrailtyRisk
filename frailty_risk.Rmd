...
Because patients with complete data are likely different from patients who
survived critical illness but were unable to complete the followup assessment,
we will use multiple imputation to account for missing data and reduce the
likelihood of bias due to these differences. Specifically, we'll use predictive
mean matching with 10 imputations, via the `Hmisc::aregImpute()` and
`Hmisc::fit.mult.impute()` functions.
