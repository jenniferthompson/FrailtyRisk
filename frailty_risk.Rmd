...
## xbreak: how far apart X axis breaks should be
## ylabel: Y axis label (none by default)
...
## text_size: font size for axis text
## linewidth: width for point estimates (continuous)
## point_size: size for pointranges
## label_title: whether label should be the title or subtitle
##   (subtitle used when we're trying to fake faceting by month, but include
##    continuous and integer covariates)
predict_cfs_vals <- function(
  ...
  ## These all make it easier to do versions for within the report and for
  ## publication/presentation; defaults to values for within-report
  title_size = 1.2,
  subtitle_size = 1,
  text_size = 0.8,
  line_size = 0.5,
  ...
  gg <- ggplot(data = predvals, aes_string(x = covar, y = "yhat")) +
    scale_y_continuous(name = ylabel, limits = c(1, 7), breaks = c(1, 3, 5, 7)) +
    theme_minimal() +
    theme(plot.title = element_text(face = "bold", size = rel(title_size)),
          plot.subtitle = element_text(face = "bold", size = rel(subtitle_size)),
          axis.text = element_text(size = rel(text_size)),
          panel.border = element_rect(colour = "grey80", fill = NA))

  ## If we're picky about X axis breaks...
  if(!is.null(xbreak)){
    if(!inherits(predvals[, covar], "numeric")){
      stop("`xbreak` must be used with a numeric covariate")
    }
    
    ## X axis limits: floor(min), ceiling(max) of X variable
    xlims <- c(min(predvals[, covar]), max(predvals[, covar]))
    
    gg <- gg +
      scale_x_continuous(
        limits = xlims, breaks = seq(xlims[[1]], xlims[[2]], xbreak)
      )
  }
    
  if(label_title){
    gg <- gg + labs(title = varlabels[covar], x = NULL, y = NULL)
  } else{
    gg <- gg +
      labs(title = use_title, subtitle = varlabels[covar], x = NULL, y = NULL)
  }
  ...
  if(use_pointrange){
    if(!both_types){
      gg <- gg +
        geom_pointrange(
          aes(ymin = lower, ymax = upper), size = point_size, colour = "#003D79"
        )
    } else{
      gg <- gg +
        geom_pointrange(
          aes(ymin = lower, ymax = upper, colour = wtd),
          position = position_dodge(dodge_width), size = point_size
        ) +
    ...
    if(!both_types){
      gg <- gg +
        ...
        geom_line(colour = "#003D79", size = line_size)
    } else{
      gg <- gg +
        geom_ribbon(aes(ymin = lower, ymax = upper, fill = wtd), alpha = 0.4) +
        geom_line(aes(colour = wtd), size = line_size) +
...
## Calculate difference between frailty at each time point
brainmind.oneobs$cfs_diff_e3 <- with(brainmind.oneobs, {
  ifelse(id %in% pts_3m, frailty_num.3 - frailty_num, NA)
})
brainmind.oneobs$cfs_diff_e12 <- with(brainmind.oneobs, {
  ifelse(id %in% pts_12m, frailty_num.12 - frailty_num, NA)
})
brainmind.oneobs$cfs_diff_312 <- with(brainmind.oneobs, {
  ifelse(id %in% pts_12m, frailty_num.12 - frailty_num.3, NA)
})

label(brainmind.oneobs$cfs_diff_e3) <- "3m vs enrollment"
label(brainmind.oneobs$cfs_diff_e12) <- "12m vs enrollment"
label(brainmind.oneobs$cfs_diff_312) <- "12m vs 3m"
...
fu_plotdf <- brainmind.fu %>%
  ...
  left_join(dplyr::select(brainmind.oneobs, id, age.cat, age.enroll), by = "id")

## -- Add frailty at baseline --------------------------------------------------
all_plotdf <- brainmind.oneobs %>%
  dplyr::select(id, frailty_num, age.cat, age.enroll) %>%
...
age_frailty <- ggplot(
  data = fu_plotdf,
  aes(x = frailty_num, y = age.enroll, group = frailty_num)
) +
  facet_wrap(~ timept, ncol = 1) +
  geom_point(colour = "#003D79", alpha = 0.15,
             position = position_jitter(width = 0.25, height = 0.2)) +
  geom_boxplot(colour = "#002346", fill = NA, outlier.shape = NA) +
  scale_y_continuous(name = "Age at Enrollment") +
  scale_x_continuous(name = "CSHA Clinical Frailty Score", breaks = 1:7) +
  theme_minimal() +
  theme(strip.background = element_rect(fill = "grey90", colour = "grey80"),
        strip.text = element_text(face = "bold", size = 14),
        axis.title = element_text(face = "bold", size = 14),
...
html(
  summaryM(
    cfs_diff_e3 + cfs_diff_e12 + cfs_diff_312 ~ 1,
    continuous = 5,
    data = brainmind.oneobs
  ),
  digits = 2,
  prmsd = TRUE,
  caption = "Changes in Clinical Frailty Scores Over Time"
)
...
All models will use robust standard errors.
...
survivors. Residual vs fitted and Q-Q plots are shown below.

<details>
<summary>Visual Assumption Checks for All Models</summary>
...
This section includes code used to create and save high-resolution versions of
figures for publication and presentation.

```{r pub_figures}
## -- List of figures to save: -------------------------------------------------
## - Boxplot, age categories vs frailty at both follow-up time points
## - Combined plot for publication: ADL, CFS and SOFA in-hospital vs frailty and
##    3 and 12m (IPAW models only)
## - Separate plots for each risk factor for presentations

model_covar_labels_3m <-
  paste(model_covar_labels, "vs 3-Month CFS") %>% set_names(model_covars)
model_covar_labels_12m <-
  paste(model_covar_labels, "vs 12-Month CFS") %>% set_names(model_covars)

rf_xbreaks <- list(10, 2, NULL, 2, 2, 3, 2, 3, 2, 5, 5, NULL, 1) %>%
  set_names(model_covars)

## -- Separate plots, for presentations, etc -----------------------------------
rf_plots_3m <- pmap(
  list(
    covar = model_covars,
    xbreak = rf_xbreaks
  ...
  predict_cfs_vals,
  mod_wtd = "cfs_mod_3m_wtd",
  varlabels = model_covar_labels_3m,
  title_size = 1.5, text_size = 1.2, line_size = 1.5, point_size = 1.25
)
...
rf_plots_12m <- pmap(
  ...
  predict_cfs_vals,
  mod_wtd = "cfs_mod_12m_wtd",
  varlabels = model_covar_labels_12m,
  title_size = 1.5, text_size = 1.2, line_size = 1.5, point_size = 1.25
)

## Save both as PDFs, PNGs
walk2(
  .x = sprintf(
    "cfs_%s_%s.pdf",
    gsub(".", "_", model_covars, fixed = TRUE),
    c(rep(3, length(model_covars)), rep(12, length(model_covars)))
  ),
  .y = c(rf_plots_3m, rf_plots_12m),
  ggsave,
  device = "pdf", path = "pubfigures/pdf",
...
walk2(
  ...
  .y = c(rf_plots_3m, rf_plots_12m),
  ggsave,
  device = "png", path = "pubfigures/png",
  width = 7, height = 5, units = "in", dpi = 400
)

## -- Combined plot for manuscript: LHS = 3m, RHS = 12m ------------------------
## Which risk factors to include?
rf_include_pub <- c("adl.e", "frailty_num", "mean.modsofa.icu")
rf_ylabels <- c(" ", "Adjusted Clinical Frailty Score", " ")

rf_plots_3m_combine <- pmap(
  list(
    covar = rf_include_pub,
    use_title = c("3 Months\n", "\n", "\n"),
    xbreak = rf_xbreaks[rf_include_pub],
    ylabel = rf_ylabels
  ),
  predict_cfs_vals,
  mod_wtd = "cfs_mod_3m_wtd",
  varlabels = model_covar_labels,
  label_title = FALSE,
  title_size = 1.5, subtitle_size = 1.2, text_size = 0.8,
  line_size = 1.25, point_size = 0.5
)

rf_plots_12m_combine <- pmap(
  list(
    covar = rf_include_pub,
    use_title = c("12 Months\n", "\n", "\n"),
    xbreak = rf_xbreaks[rf_include_pub],
    ylabel = rep(" ",)
  ),
  predict_cfs_vals,
  mod_wtd = "cfs_mod_12m_wtd",
  varlabels = rep("", length(model_covars)) %>% set_names(model_covars),
  label_title = FALSE,
  title_size = 1.5, subtitle_size = 1.2, text_size = 0.8,
  line_size = 1.25, point_size = 0.5
)

pdf("pubfigures/pdf/cfs_riskfactors_pub.pdf", width = 6, height = 8)
multiplot(
  plotlist = c(rf_plots_3m_combine, rf_plots_12m_combine),
...
pwalk(
  list(
    filename = list("frailty_age.pdf", "age_frailty.pdf",
                    "frailty_age.png", "age_frailty.png"),
    plot = list(frailty_age, age_frailty, frailty_age, age_frailty),
    device = c(rep("pdf", 2), rep("png", 2)),
    path = c(rep("pubfigures/pdf", 2), rep("pubfigures/png", 2)),
    height = c(5, 7, 5, 7)
  ),
  ggsave,
  width = 7, units = "in", dpi = 400
)
