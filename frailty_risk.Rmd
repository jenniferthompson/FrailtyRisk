...
1. **Models weighted by the inverse probability of having follow-up**, in which
...
A brief outline of the steps for inverse probability weighting (IPW):
...
prob_cohort <- function(
  ...
  return(list("probs" = probs, "wts" = wts))
}

probs_3 <- prob_cohort(cohort_var = "cohort_3m")$probs
probs_12 <- prob_cohort(cohort_var = "cohort_12m")$probs

## Get weights for each time point
brainmind.oneobs$wts_3m <- prob_cohort(cohort_var = "cohort_3m")$wts
brainmind.oneobs$wts_12m <- prob_cohort(cohort_var = "cohort_12m")$wts

## Plot, summarize weights for patients in each cohort at each time point
wts_df <- brainmind.oneobs %>%
  dplyr::select(id, starts_with("cohort"), starts_with("wts")) %>%
...
wts_hist <- ggplot(data = wts_df, aes(x = wts)) +
  facet_wrap(~ timept) +
  geom_histogram() +
...
wts_summary <- wts_df %>%
  group_by(timept) %>%
  summarise(sumwts = sum(wts, na.rm = TRUE),
            maxwt = max(wts, na.rm = TRUE))

```

We have `r sum(!is.na(brainmind.oneobs$wts_3m))` patients with no missingness
in the variables used to fit the IPW models; our weights sum to
...
```{r model_assume}
par(mfrow = c(1, 2))
lm_diagnostics(cfs_mod_3m_unwtd, outcomeString = "CFS, 3m", titleString = "3m Frailty")
lm_diagnostics(cfs_mod_12m_unwtd, outcomeString = "CFS, 12m", titleString = "12m Frailty")
par(mfrow = c(1, 1))
...
less education and a *lower* modified mean SOFA score in the ICU are associated
with worse frailty at both 3 and 12 months after discharge among ICU survivors.
In addition, discharge to a facility is associated with worse frailty at 3
months, but not at 12 months; whether this is due to a true easing of effect or
dropout of the most frail patients is a matter for further consideration. Among
those who survived and were assessed at 12 months, older age is an additional
risk factor for worse frailty at that time point.
...
print_html(cfs_results_3m_unwtd)

```

### `r figure_nums("plots_3m")`

Values on Y axis are predicted CSHA Frailty Scores for a given level of the X
axis, adjusted to the median or mode of all other covariates.
         
```{r plots_3m}
multiplot(
  plotlist = cfs_plots_3m_unwtd,
...
print_html(cfs_results_12m_unwtd)
